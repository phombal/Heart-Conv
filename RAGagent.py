...
VECTOR_STORE_ID = os.environ.get("HF_VECTOR_STORE_ID")

def ensure_vector_store_loaded() -> str:
    """
    Ensure we have a vector store populated with the Knowledge markdown files.
    If HF_VECTOR_STORE_ID is already set, reuse it. Otherwise create a new store,
    upload the files, and cache its ID for this process.
    """
    global VECTOR_STORE_ID
    if VECTOR_STORE_ID:
        return VECTOR_STORE_ID

    vector_store = client.vector_stores.create(name="HF Knowledge Base")
    print(f"Created vector store: {vector_store.id}")

    all_md_files = list(KNOWLEDGE_DIR.glob("*.md"))
    if not all_md_files:
        raise FileNotFoundError(f"No markdown files found in {KNOWLEDGE_DIR}")

    md_files = [p for p in all_md_files if p.stat().st_size > 0]
    skipped = [p.name for p in all_md_files if p.stat().st_size == 0]
    if skipped:
        print(f"Skipping empty knowledge files: {', '.join(skipped)}")

    if not md_files:
        raise FileNotFoundError("All knowledge files are empty; cannot build vector store.")

    for md_file in md_files:
        with md_file.open("rb") as stream_handle:
    ...
    VECTOR_STORE_ID = vector_store.id
    print(
        "Vector store populated with knowledge files. "
        "Persist the ID in HF_VECTOR_STORE_ID for reuse."
    )
    return VECTOR_STORE_ID
...
@function_tool
def retrieveInformation(query: str):
    """
    Retrieve relevant passages from the heart failure knowledge base.
    """
    if not OPENAI_API_KEY:
        return "OPENAI_API_KEY not configured."

    vector_store_id = ensure_vector_store_loaded()
...
class RAGAgent:
    def __init__(self, scenario_str: str):
        ...
        self.recommendation_agent = Agent(
            ...
            instructions="Use the retrieveInformation tool to get information from the knowledge base. Then look up information as relevant to the patient's condition and the recommendation.",
            tools=[
                retrieveInformation,
                checkNotEmergency,
                self.arb_checker.as_tool(tool_description="Check if the patient's condition violates any of the contraindications or HOLD criteria for ARBs.", tool_name="check_arb"),
                self.arni_checker.as_tool(tool_description="Check if the patient's condition violates any of the contraindications or HOLD criteria for ARNI.", tool_name="check_arni"),
                self.aldosterone_antagonist_checker.as_tool(tool_description="Check if the patient's condition violates any of the contraindications or HOLD criteria for aldosterone antagonists.", tool_name="check_aldosterone_antagonist"),
                self.beta_blocker_checker.as_tool(tool_description="Check if the patient's condition violates any of the contraindications or HOLD criteria for beta blockers.", tool_name="check_beta_blocker"),
                self.sgc_checker.as_tool(tool_description="Check if the patient's condition violates any of the contraindications or HOLD criteria for SGC.", tool_name="check_sgc"),
                self.sglt2_checker.as_tool(tool_description="Check if the patient's condition violates any of the contraindications or HOLD criteria for SGLT2.", tool_name="check_sglt2"),
                self.hydralazine_checker.as_tool(tool_description="Check if the patient's condition violates any of the contraindications or HOLD criteria for hydralazine.", tool_name="check_hydralazine"),
                physicianApproval,
            ],
        )
