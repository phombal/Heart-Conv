...
**Solution**: Set your OpenAI API key (see Environment Setup step 4)

### Issue: `Strict JSON schema validation errors`

**Solution**: This should be fixed in the current version. If issues persist, the schema has been updated to use list-based structures compatible with OpenAI's strict mode.

### Issue: Slow simulation runtime

**Expected**: Each conversation takes ~1-2 minutes. Full simulation of 41 scenarios takes 30-60 minutes.

**To speed up**: Reduce the number of scenarios by filtering in `simulation.py`
...
- OpenAI Agents SDK
- Clinical guidelines from ACC/AHA Heart Failure Guidelines
